...
main_application.title('Mpat text editor')
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        ...
        characters = len(text_editor.get(1.0,'end-1c').replace(' ','')) # replace function is used for not counting space as a form of a character
...
file.add_command(label='Save As',image=new_icon, compound=tk.LEFT, accelerator='Ctrl+Alt+N', command=save_as)
...
view.add_checkbutton(label='Status Bar', onvalue=True, offvalue=False,variable=show_statusbar, image=statusbar_icon, compound=tk.LEFT, command=hide_statusbar)
